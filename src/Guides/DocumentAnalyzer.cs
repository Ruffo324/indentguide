...
namespace IndentGuide.Guides
{
    ...
    public sealed class LineSpan : IEquatable<LineSpan>
    {
...
#endif
            HashSet<LineSpan> result = new HashSet<LineSpan>();
            Queue<LineSpan> queue = new Queue<LineSpan>();
            queue.Enqueue(line);
            while (queue.Any())
            {
                LineSpan ls = queue.Dequeue();
                if (result.Add(ls) && ls._LinkedLines != null)
                    foreach (LineSpan ls2 in ls._LinkedLines)
                    {
        ...
        {
            foreach (LineSpan line in LinkedLinesInternal(existingLine, cancel))
            {
        ...
        {
            foreach (LineSpan line in LinkedLinesInternal(removeLine, cancel))
            {
                if (line._LinkedLines != null) line._LinkedLines.Remove(removeLine);

                if (removeLine._LinkedLines != null) removeLine._LinkedLines.Remove(line);
            }
        }

        public static void MoveLinks(LineSpan fromLine, LineSpan toLine, CancellationToken cancel)
        {
            HashSet<LineSpan> links = LinkedLinesInternal(fromLine, cancel);
            links.Remove(fromLine);

            foreach (LineSpan line in links)
            {
                if (line._LinkedLines != null) line._LinkedLines.Remove(fromLine);

                if (fromLine._LinkedLines != null) fromLine._LinkedLines.Remove(line);
            }
    ...
    public class DocumentAnalyzer
    {
        ...
        {
            List<LineSpanChunk> intersection = new List<LineSpanChunk>();
            lock (Chunks)
            {
                int start = 0;
                foreach (LineSpanChunk c in Chunks)
                {
                    int end = start + c.TextLineCount;
                    if (lastLine >= start && firstLine <= end) intersection.Add(c);
                    start = end;
                }
            }
        ...
        {
            List<LineSpanChunk> chunks = new List<LineSpanChunk>();
            lock (Chunks)
            {
        ...
        public async Task Reset()
        {
            ...
            try
            {
                ...
                if (Snapshot == null || snapshot.Length < Snapshot.Length) FindLongestLine(snapshot, cancel);
        ...
        {
            if (Snapshot != OriginalSnapshot.TextBuffer.CurrentSnapshot) await Reset();
        }
        ...
        private void SetLineInfo_Performance(
...
#endif
            string contentType = snapshot.ContentType != null ? snapshot.ContentType.TypeName : null;
            ...
            for (int lineNumber = firstLine; lineNumber <= lastLine; ++lineNumber)
            {
                ...
                LineInfo curLine = lineInfo[lineNumber - firstLine];
                ...
                int normalizedLength = text.ActualLength(TabSize);
                if (normalizedLength > LongestLine) LongestLine = normalizedLength;
        ...
        private void ValidateChunks(IEnumerable<LineInfo[]> chunks)
        {
            ...
            uint lineNumber = 0;
            foreach (LineInfo line in chunks.SelectMany(i => i))
            {
        ...
        private List<LineSpanChunk> GetLineSpans_Performance(IList<LineInfo[]> chunkInfo, CancellationToken cancel) {
            ...
            List<LineSpanChunk> result = new List<LineSpanChunk>(chunkInfo.Count);
            LineSpanBuilder builder = new LineSpanBuilder(IndentSize, Behavior);
            foreach (LineInfo[] chunk in chunkInfo)
            {
                foreach (LineInfo line in chunk)
                {
                    cancel.ThrowIfCancellationRequested();
        ...
        private struct LineInfo
        {
            ...
            public int Number
            {
                get => (int) (_packedValue & _numberMask);
                set
                {
            ...
            public bool HasText
            {
                get => (_packedValue & _hasTextMask) != 0;
                set
                {
                    if (value)
            ...
            {
                get => (_packedValue & _skipLineMask) != 0;
                set
                {
                    if (value)
            ...
            public int TextAt
            {
                get => (int) ((_packedValue & _textAtMask) >> _textAtShift);
                set
                {
        ...
        private class LineSpanChunk
        {
            ...
            public ICollection<LineSpan> Lines { get; }
        ...
        private class LineSpanBuilder
        {
            ...
            {
                List<LineSpan> res = _completedSpans;
                _completedSpans = new List<LineSpan>();
            ...
            public void AddNextLine(LineInfo line)
            {
                ...
                else if (line.HasText && _previousLine.HasText) FromTextToText(line.Number, line.TextAt);
                _previousLine = line;
            }
            ...
            public void NoMoreLines()
            {
                if (!_options.ExtendInwardsOnly)
                    for (int i = 0; i < _activeSpans.Count; ++i)
                    {
                        ...
                        if (span.FirstLine > span.LastLine) _activeSpans[i] = null;
                    }
            }

            private IndentSet GetIndents(int textAt)
            {
                if (textAt > 128) textAt = 128;

                IndentSet set = _setCache[textAt];
                ...
                if (_options.VisibleAligned)
                    for (int i = 0; i < textAt; i += _indentSize)
                        set.Set(i);
                if (_options.VisibleUnaligned || textAt % _indentSize == 0) set.Set(textAt);
            ...
            {
                LineSpan span = new LineSpan(line, line, indent, lineType);
                for (int i = 0; i < _activeSpans.Count; ++i)
            ...
            )
            {
                bool extendToTop = _options.VisibleEmpty &&
                                   ...
                                   line > 0;
                foreach (int indent in indents.GetAll())
                {
                    if (indent > textAt) continue;

                    if (indent == textAt && !_options.VisibleAtTextEnd) continue;
            ...
            {
                IndentSet indents = GetIndents(textAt);
                ...
                for (int i = 0; i < _activeSpans.Count; ++i)
                {
                    ...
                    if (finish)
                    {
                        ...
                        {
                            if (!_options.ExtendInwardsOnly || span.Indent == textAt) span.LastLine = line - 1;
                            if (span.LastLine >= span.FirstLine) _completedSpans.Add(span);
                        }
                    }
                }
                ...
                {
                    if (_options.VisibleEmptyAtEnd && !_options.VisibleAtTextEnd) indents.Set(textAt);
                    foreach (LineSpan span in _activeSpans)
                    {
                        if (span == null) continue;
            ...
            private void FromTextToEmpty(int line)
            {
                ...
                int textAt = _previousLine.TextAt;
                IndentSet indents = GetIndents(textAt);
            ...
            {
                IndentSet indents = GetIndents(textAt);
                int prevTextAt = _previousLine.TextAt;
                ...
                for (int i = 0; i < _activeSpans.Count; ++i)
                {
                    ...
                    {
                        span.LastLine = line;
                        if (span.Indent != textAt || _options.VisibleAtTextEnd) finish = false;
                    }
                ...
                if (_lastEmptyToTextLine < 0) _lastEmptyToTextLine = line;
            }
            ...
            private void FromNoSkipToSkip(int line)
            {
                for (int i = 0; i < _activeSpans.Count; ++i)
                {
                    ...
                    LineSpan newSpan = new LineSpan(line, line, span.Indent, span.Type);
            ...
            private void FromSkipToNoSkip(int line, bool hasText, int textAt)
            {
                for (int i = 0; i < _activeSpans.Count; ++i)
                {
                    ...
                    if (hasText)
                    {
                        if (span.Indent > textAt || span.Indent == textAt && !_options.VisibleAtTextEnd) remove = true;
                    }
                ...
                {
                    IndentSet indents = GetIndents(_previousLine.TextAt);
                    for (int i = 0; i < _activeSpans.Count; ++i)
                    {
                        LineSpan span = _activeSpans[i];
                        if (span == null) continue;

                        if (indents.Remove(span.Indent)) span.FirstLine = span.LastLine = line;
                    }
        ...
        internal struct IndentSet
        {
            ...
            public void Set(int indent)
            {
                ...
                else if (indent < Count * 2) _value2 |= Masks[indent - Count];
            }
