...
namespace IndentGuide.Dialogs.Controls
{
    public partial class LineTextPreview : UserControl
    {
        ...
        public override void Refresh()
        {
            ...
            {
                FakeSnapshot snapshot = new FakeSnapshot(Text);
        ...
        private Pen GetLinePen(int formatIndex, out bool isGlowing)
        {
            ...
            Pen pen = new Pen(format.LineColor, (float) format.LineStyle.GetStrokeThickness());

            pen.StartCap = LineCap.Flat;
            pen.EndCap = LineCap.Flat;

            float[] pattern = format.LineStyle.GetDashPattern();
            if (pattern == null)
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (StringFormat sf = new StringFormat())
            {
                ...
                string testString = VisibleWhitespace ? "················{" : "                {";
                Region[] rgns = e.Graphics.MeasureCharacterRanges(testString, Font, rect, sf);
                RectangleF bounds = rgns[0].GetBounds(e.Graphics);
            ...
            try
            {
                if (Analysis == null || Analysis.Snapshot == null) return;
                ITextSnapshot snapshot = Analysis.Snapshot.TextBuffer.CurrentSnapshot;
                ...
                using (SolidBrush brush = new SolidBrush(foreColor))
                {
                    foreach (ITextSnapshotLine line in snapshot.Lines)
                    {
                        ...
                        if (VisibleWhitespace) text = text.Replace(' ', '·');
                ...
                IEnumerable<LineSpan> analysisLines = Analysis.GetLines(0, snapshot.LineCount);
                ...
                foreach (LineSpan line in analysisLines)
                {
                    ...
                    bool glow;
                    using (Pen pen = GetLinePen(line.FormatIndex, out glow))
                    {
                        if (pen != null)
                        {
                            if (glow)
                                using (SolidBrush transparentBrush = new SolidBrush(Color.FromArgb(24, pen.Color)))
                                {
                                    for (int i = 1; i < LineStyle.Thick.GetStrokeThickness(); ++i)
                                        e.Graphics.FillRectangle(transparentBrush, left - i, top, i + i + 1,
                                            bottom - top);
                                }
